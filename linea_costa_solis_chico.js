...
function getLandsatCollection(anio) {
  ...
  return col.filterBounds(aoi)
           .filter(ee.Filter.lt('CLOUD_COVER', cloudMaxA));
}
...
var lineas1990_2022 = ee.FeatureCollection(aniosLandsatA.map(function(anio) {
  ...
  return ee.Algorithms.If(
    ...
    (function() {
      ...
      var waterMask = ndwiMedian.gt(thresholdA);
      ...
      return filtrado.map(function(f) {
        var geom = f.geometry().simplify(simplTolerance);
...
var lineas2013_2017 = ee.FeatureCollection(aniosLandsatB.map(function(anio) {
  ...
  var coleccion = col.filterBounds(aoi)
  ...
  return ee.Algorithms.If(
    ...
    (function() {
      var ndwiMedian = coleccion.select('NDWI').median().clip(aoi);
      var waterMask = ndwiMedian.gt(thresholdB);
...
var lineas2017_2024 = ee.FeatureCollection(aniosSentinel.map(function(anio) {
  ...
  return ee.Algorithms.If(
    ...
    (function() {
      var ndwiMedian = coleccion.select('NDWI').median().clip(aoi);
      var waterMask = ndwiMedian.gt(thresholdC);
      ...
      return filtrado.map(function(f) {
        var geom = f.geometry().simplify(simplToleranceC);
        return ee.Feature(geom, {
          'anio': anio,
          'area_m2': f.get('area_m2'),
          'fuente': 'Sentinel2'
